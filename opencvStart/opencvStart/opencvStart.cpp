// opencvStart.cpp : 콘솔 응용 프로그램에 대한 진입점을 정의합니다.

/*
void init(MyImage *m)
precondition : none
postcondition : settiing square_len=20; iSinceKFInit=0;

void col2origCol(int hsv[3], int bgr[3], Mat src){
precondition :  hsv, bgr, src is not null.
postcondition : set the value of hsv, bgr to average value.

void printText(Mat src, string text){
precondition : src, text is not null.
postcondition : put the text on the screen.


void waitForPalmCover(MyImage* m)
precondition : MyImage is not null.
postcondition : none

int getMedian(vector<int> val){
precondition : val is not null.
postcondition : set median is mid value
return : mid value of val

void getAvgColor(MyImage *m,My_ROI roi,int avg[3]){
precondition : value is not null.
postcondition : set median color value

void average(MyImage *m){
precondition : value is not null.
postcondition : setaverage

void initTrackbars(){
precondition : none
postcondition : initTrackbar

void normalizeColors(MyImage * myImage){
precondition : parameter is not null
postcondition : normalizecolor

void produceBinaries(MyImage *m){	
precondition : parameter is not null
postcondition : produce the value binaries code

void initWindows(MyImage m){
precondition : parameter is not null
postcondition : initialize the wintodows

void showWindows(MyImage m){
precondition : value is not null
postcondition : show the windows

int findBiggestContour(vector<vector<Point> > contours){
precondition : value is not null
postcondition : find the value of the finger
return : finger value

void myDrawContours(MyImage *m,HandGesture *hg){
precondition : value is not null
postcondition : Draw the contours of the hands move

void makeContours(MyImage *m, HandGesture* hg){
precondition : value is not null
postcondition : make the contours

int _tmain(int argc, _TCHAR* )
precondition : none
postcondition : start program
